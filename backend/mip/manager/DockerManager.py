...
from mip.data import ModelSnapshot, Result
from mip.manager.utils.computeUtils import update_instance_status_rest
from mip.utils.utils import write_to_file, get_timestamp, remove_ansi_escape_tags, write_json_file, \
    send_email
    ...
    def save_container_state(self, context: Union[ModelSnapshot, Result]):
        ...
        if context.success:
            ...
            container.commit(repository=new_container_image_name,
                             tag=f'latest',
                             message=f"MIP: trained model snapshot",
