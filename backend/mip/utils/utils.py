...
def clean_name(name_to_clean):
    # Check if valid method_name is provided
...
def send_email(to_address, subject, body, attachments=None, from_address="Matthias.Greiner@stud.uni-heidelberg.de"):
    # Logging
    logger = logging.getLogger('backend.' + __name__)
    logger.info("Sending Email to {}, Subject: {}".format(to_address, subject))

    # Authenticate
    server = smtplib.SMTP_SSL("SMTP_server_URL_here", 465)
    server.ehlo()
    # TODO improve credential management
    server.login('Login_email_here', 'Login_Password_here')

    # Compose Email
    message = MIMEMultipart("mixed")
    message["Subject"] = f" [MIP - Notification]: {subject}"
